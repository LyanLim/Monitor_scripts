...
for idx, val in enumerate(data):
	...
	id = getCmd(snmpEngine, CommunityData('ViettelMS1NMS'), UdpTransportTarget((data[idx]['dcnIP'], 161)), ContextData(), ObjectType(ObjectIdentity(bandwidth_oid)), cbFun=cbFun)
	logger.info('get the current bandwidth -> %s', data[idx]['dcnIP'])
	data[idx]['reqID'] = id
...
for idx, val in enumerate(data):
	if data[idx]['reqID'] in dicSnmpResponse:
		data[idx]['curbw'] = int(dicSnmpResponse[data[idx]['reqID']])
		logger.info('bandwidth of %s : %d', data[idx]['dcnIP'], data[idx]['curbw'])
